...
import { fetchProblemBySlug, submitSolution } from "@/lib/api";
...
export default function ProblemSolvePage() {
  ...
  useEffect(() => {
    if (slug) {
      fetchProblemBySlug(slug as string).then(setProblem).catch(console.error);
    }
  }, [slug]);
          ...
          <div className="w-2/5 p-6 overflow-y-auto bg-white border-r border-gray-200">
            ...
            {problem.sampleTestCases?.map((tc: any, idx: number) => (
              <div key={idx} className="mt-6 bg-gray-50 p-4 rounded-lg">
                <h3 className="font-semibold text-sm text-gray-900 mb-2">Example {idx + 1}</h3>
                <div className="space-y-2 text-sm font-mono">
                  ...
                  {tc.explanation && <div><span className="text-gray-500">Explanation:</span> {tc.explanation}</div>}
                </div>
              </div>
            ))}
          </div>
              ...
              <div className="h-48 bg-[#1e1e1e] border-t border-[#3e3e3e] p-4 overflow-y-auto">
                ...
                {output.status === "Accepted" ? (
                  <div className="text-green-400 font-mono text-sm">
                    ✓ Accepted <span className="text-gray-500 ml-2">Runtime: {output.runtime}s</span>
                  </div>
                ) : (
                  <div className="text-red-400 font-mono text-sm whitespace-pre-wrap">
                    ✗ {output.status}
                    {output.errorMessage && <div className="mt-2 text-gray-300">{output.errorMessage}</div>}
                  </div>
                )}
              </div>
            )}
